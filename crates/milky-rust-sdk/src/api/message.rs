...
use milky_types::common::MessageScene;
use milky_types::message::in_coming::IncomingMessage;
use milky_types::message::out_going::OutgoingSegment;
...
/// 撤回私聊消息的请求参数
#[derive(Serialize)]
pub struct RecallPrivateMessageRequest {
    /// 好友 QQ 号
    pub user_id: i64,
    /// 消息序列号
    pub message_seq: i64,
...
/// 获取历史消息记录的请求参数
#[derive(Serialize)]
pub struct GetHistoryMessageRequest {
    /// 消息所属的场景
    pub message_scene: MessageScene,
    /// 消息所属的好友QQ号或群号
    pub peer_id: i64,
    /// 起始消息的序列号 (`message_seq`)
    /// 如果不提供此参数，则通常从最新的消息开始获取可选
...
#[derive(Serialize)]
pub struct GetResourceTempUrlRequest {
    /// 资源ID
...
/// 获取合并转发消息内容的请求参数
#[derive(Serialize)]
pub struct GetForwardedMessagesRequest {
    /// 转发消息ID
...
/// 标记消息为已读的请求参数
#[derive(Serialize)]
pub struct MarkMessageAsReadRequest {
    /// 消息所属的场景
    pub message_scene: MessageScene,
    /// 好友QQ号或群号
    pub peer_id: i64,
...
impl MilkyClient {
    ...
    pub async fn send_private_message(
        ...
        let params = SendPrivateMessageRequest { user_id, message };
    ...
    pub async fn send_group_message(
        ...
        let params = SendGroupMessageRequest { group_id, message };
    ...
    /// * `message_scene`: 消息场景
    /// * `peer_id`: 好友QQ号或群号
    /// * `start_message_seq`: 可选的起始消息序列号若为 `None`，则从最新消息开始
    /// * `limit`: 可选的获取消息数量上限若为 `None`，则默认为20条
    ///
    /// # 返回
    ...
    pub async fn get_history_messages(
        ...
        let params = GetHistoryMessageRequest {
            message_scene,
            peer_id,
            start_message_seq,
            limit,
    ...
    pub async fn get_resource_temp_url(
        ...
        let params = GetResourceTempUrlRequest {
    ...
    pub async fn get_forwarded_messages(
        ...
        let params = GetForwardedMessagesRequest {
    ...
    pub async fn recall_private_message(&self, user_id: i64, message_seq: i64) -> Result<()> {
        let params = RecallPrivateMessageRequest {
            user_id,
            message_seq,
        };
    ...
    pub async fn recall_group_message(&self, group_id: i64, message_seq: i64) -> Result<()> {
        let params = RecallGroupMessageRequest {
            group_id,
            message_seq,
        };
    ...
    /// * `peer_id`: 好友QQ号或群号
    /// * `message_seq`: 要标记为已读的消息序列号该消息及更早的消息将被标记为已读
    /// # 返回
    /// 成功则返回 `Ok(())`
    pub async fn mark_message_as_read(
        ...
        let params = MarkMessageAsReadRequest {
            message_scene,
            peer_id,
            message_seq,
        };
